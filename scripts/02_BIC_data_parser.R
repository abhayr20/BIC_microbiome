...
tcga_clinical <- read_tsv("rawdata/TCGA-BIC-clinical.tsv")                      # clinical parameters 
tcga_meta     <- read.csv("rawdata/Metadata-TCGA-Kraken-17625-Samples.csv")     # metagenomic metadata, X is a unique patient sample
tcga_kraken   <- read.csv("rawdata/Kraken-TCGA-Raw-Data-17625-Samples.csv")     # metagenomic cnt_mxt
tcga_rnaseq   <- readRDS(file = "rawdata/tcga_brca.RDS")                     # BRCA RNAseq data object
###################################################################
...
brca_meta <- tcga_meta %>%
  ...
  distinct(case_uuid, .keep_all = TRUE)

# Use TCGAutils to get patient IDs using case UUIDs
brca_map <- UUIDtoBarcode(unique(brca_meta$case_uuid))     
brca_clinical <- tcga_clinical[tcga_clinical$`Patient ID` %in% brca_map$submitter_id , ]

brca_map <- brca_map %>%
  left_join(brca_clinical[, c("Patient ID", "Diagnosis Age", "Race Category", "Sex", "Subtype")],
            by = c("submitter_id" = "Patient ID"))
...
map_genes <- data.frame(ENGS=tcga_rnaseq@rowRanges@elementMetadata@listData$gene_id,
                       HGSV=tcga_rnaseq@rowRanges@elementMetadata@listData$gene_name)
...
#save.image("rdata/data_parser.RData")
#load("rdata/data_parser.RData")
