...
tcga_kraken_bic <- tcga_kraken_bic %>% 
  column_to_rownames("X") %>%
  t() %>%
  as.data.frame()
microbes <- rownames(tcga_kraken_bic)
rownames(tcga_kraken_bic) <- sprintf("Otu%04d", 1:nrow(tcga_kraken_bic))

# Create taxonomic classification matrix
extract_parts <- function(strings) {
  result <- data.frame(otu = sprintf("Otu%04d", 1:nrow(tcga_kraken_bic)), 
...
# Write parsed files
write.csv(tcga_kraken_bic, file = "cleaned_data/otu_mat.csv", row.names = FALSE)
write.csv(tcga_meta_bic2, file = "cleaned_data/sample_mat.csv", row.names = FALSE)
