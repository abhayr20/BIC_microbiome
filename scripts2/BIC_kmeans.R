...
########################################################################
# Perform PCA
pca_result <- prcomp(mb_sig, scale = TRUE)
...
plot(cumulative_var, type = "b", xlab = "Number of Principal Components", ylab = "Cumulative Variance Explained")

# Biplot
fviz_pca_biplot(pca_result, repel = TRUE, geom = c("point", "text"))

# PCA Scores
scores <- as.data.frame(pca_result$x)
rownames(scores) <- rownames(mb_sig)

# Add cluster labels to the scores (replace 'cluster_column' with the actual column name containing cluster labels)
# scores$Cluster <- cluster_column

# Save PCA scores to a file (you can choose a different format if needed)
write.csv(scores, file = "pca_scores.csv", row.names = TRUE)
########################################################################
